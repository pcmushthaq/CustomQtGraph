 ...
 */
void LineNode::updateGeometry(const QRectF &bounds, const QList<qreal> &samples, size_t maxPointCount)
{
    // auto actualPointCount=(maxPointCount<samples.size())?maxPointCount:samples.size();
    // m_geometry.allocate(actualPointCount * 2);
    ...
    // float dx = w /maxPointCount ;

    // LineVertex *v = (LineVertex *) m_geometry.vertexData();
    // for (int i=0; i<actualPointCount; ++i) {
    //     v[i*2+0].set(x + dx * i, y + samples.at(i).y() * h, 0);
    //     v[i*2+1].set(x + dx * i, y + samples.at(i).y() * h, 1);
    // }
